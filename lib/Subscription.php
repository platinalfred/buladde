...
class Subscription extends Db {
	...
	protected static $db_fields = array("id", "amount", "person_id", "subscription_year", "paid_by", "received_by", "date_paid");
	...
	public function findMemberSubscriptions($pno){
		$result_array = $this->getarray(self::$table_name, "", "person_id=".$pno, "");
		return !empty($result_array) ? $result_array : false;
	}
	...
	public function isSubscribedForYear($pno, $year){
		$result = $this->getrec(self::$table_name, "subscription_year=".$year." AND person_id=".$pno, "", "");
		if($result > 0){
			return true;
		}
		return false;
