...
class Accounts extends Db {
	...
	public function findAllTransactions($where = "", $inner_where = ""){
		...
		$table = "`transaction` JOIN (SELECT `person`.`person_number`, `person`.`id` `person_id`, `firstname`, `lastname`, `othername`, `member`.`id` `member_id` FROM `member` JOIN `person` ON `member`.`person_number` = `person`.`id` $inner_where)`person` ON `transaction`.`person_number` = `person`.`person_id` JOIN accounts ON `transaction`.`person_number` = `accounts`.`person_number`";
	...
	public function findOpeningBalance($where = "", $inner_where = ""){
		...
		$table = "`transaction` JOIN (SELECT `person`.`person_number`, `person`.`id` `person_id`, `firstname`, `lastname`, `othername` FROM `member` JOIN `person` ON `member`.`person_number` = `person`.`id` $inner_where)`person` ON `transaction`.`person_number` = `person`.`person_id` JOIN accounts ON `transaction`.`person_number` = `accounts`.`person_number`";
