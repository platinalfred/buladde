...
$figures['shares'] = $dashboard->getAmountOfShares("transaction_type = 4 AND (`transaction_date` BETWEEN '".$start_date."' AND '".$end_date."') $where_person");

//sum of deposits
$figures['deposits'] = $dashboard->getSumOfDeposits("transaction_type = 1 AND (`transaction_date` BETWEEN '".$start_date."' AND '".$end_date."') $where_person");
...
$figures['expected_payback'] = $loan->findExpectedPayBackAmount("(`loan_date` BETWEEN '".$start_date."'AND '".$end_date."') $where_person");

//sum of paid amount
$figures['amount_paid'] = $loan->findAmountPaid("loan_id IN  (SELECT id FROM `loan` WHERE `loan_date` BETWEEN '".$start_date."'AND '".$end_date."') AND (`transaction_date` BETWEEN '".$start_date."' AND '".$end_date."') $where_person");
